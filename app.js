...
async function init() {
  // Translate an install/launch query (?team=… / ?view=standings) into the hash
  // route the SPA uses — before the first render reads the hash.
  bootRouteFromQuery();
...
// On install, both iOS Safari (16.4+) and Android Chrome open the MANIFEST's
// start_url and ignore the address-bar URL/hash — and iOS only reads a REAL,
// fetchable, same-origin .webmanifest FILE (a runtime blob: manifest is ignored,
// which is why the earlier attempt fell back to home). So we keep one real
// manifest file per context, each with its own start_url, and point
// <link rel=manifest> at the right one for the current route. The leaderboard +
// per-team files are generated by build-division.py; manifest.webmanifest is our
// own home.
function manifestFileForRoute(path, from) {
  if (path === "standings") return "manifest-standings.webmanifest";
  const ctx = teamContextId(path) ?? teamContextId(from);
  if (ctx != null && ctx !== TEAM_ID && isDivisionTeam(ctx)) return `manifest-team-${ctx}.webmanifest`;
  return "manifest.webmanifest";
}
function updateInstallManifest(path, from) {
  const link = document.querySelector('link[rel="manifest"]');
  if (link) link.setAttribute("href", manifestFileForRoute(path, from));
}

// Those manifests use a query-param start_url (e.g. "./?team=11294") so each
// context installs as its own app — a hash-only start_url collapses to one
// identity on Android (the fragment is stripped from the manifest id). The
// installed app launches at that query; translate it to the hash route the SPA
// already understands, then clean the URL. Runs once at boot, before render.
function bootRouteFromQuery() {
  const p = new URLSearchParams(location.search);
  const team = p.get("team");
  const view = p.get("view");
  let route = null;
  if (team && /^\d+$/.test(team)) route = `#team/${team}`;
  else if (view === "standings") route = "#standings";
